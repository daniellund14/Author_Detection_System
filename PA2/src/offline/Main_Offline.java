...
public class Main_Offline {
	...
	InterruptedException {
		...
		MRJob.job(conf, args[0], UNIGRAM_PATH, UnigramAuthorMapper.class, WordCountReducer.class, Main_Offline.class);
        MRJob.job(conf, UNIGRAM_PATH, TF_PATH, TFMapper.class, TFReducer.class, Main_Offline.class);
        ...
        MRJob.job(conf, TF_PATH, IDF_PATH, IDFMapper.class, IDFReducer.class, Main_Offline.class);
        MRJob.multipleInputsJob(conf, TF_PATH, IDF_PATH, AAV_PATH, TFIDFMapper.class, TFIDFReducer.class, Main_Offline.class);
        //System.exit(AAVJob(conf, args[4], args[5])? 0: -1);
	...
	private static Integer countAuthors(Configuration conf, String inputFile) throws IOException, ClassNotFoundException, InterruptedException {
        if(MRJob.job(conf, inputFile, AUTHOR_PATH, CountAuthorsMapper.class, CountAuthorsReducer.class, Main_Offline.class)) {
